...
with transaction.atomic():
    topic = Topic.objects.select_for_update().get(slug='waverly-jameson')
    ...
    (folder / ('waverly-layout-' + timezone.now().strftime('%Y%m%dT%H%M%S%f') + '.json')).write_text(json.dumps({'post_id': post.pk, 'content': original, 'links': topic.scenario_link_cards}, ensure_ascii=False), encoding='utf-8')
    post.content = content
    post.save(update_fields=['content', 'is_edited', 'updated_at'])
    post.refresh_from_db()
    assert post.content == content
    print('Présentation harmonisée : 5 sections, texte et portrait conservés, structure harmonisée.')
