...
with transaction.atomic():
    topic = Topic.objects.select_for_update().get(
        pk=77, slug='maelya-vu', category__slug='scenarios-a-prendre'
    )
    ...
    backup = folder / ('maelya-vu-' + timezone.now().strftime('%Y%m%dT%H%M%S%f') + '.json')
    backup.write_text(serializers.serialize('json', objects), encoding='utf-8')
    print(topic.delete())
    assert not Topic.objects.filter(pk=77).exists()
    print('Scénario Maelya Vu supprimé et absence vérifiée.')
